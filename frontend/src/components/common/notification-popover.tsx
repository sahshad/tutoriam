import { Bell, Book, Calendar, Video, Award, CheckCircle, BookOpen, DollarSign, Check, MoreVertical, Trash2, Trash2Icon } from "lucide-react"
...
import { markNotificationAsReadThunk } from "@/redux/thunks/notificationThunk"
...
function NotificationItem({ notification }: { notification: Notification }) {
  ...
  return (
    <div className={`flex gap-3 p-3 border-b ${notification.isRead ? "" : "bg-muted/20"}`}>
      <div className={`flex h-8 w-8 items-center justify-center rounded-full ${notification.isRead ? "bg-muted text-muted-foreground" : "bg-primary/10 text-primary"}`}>
        {getIcon()}
      </div>
